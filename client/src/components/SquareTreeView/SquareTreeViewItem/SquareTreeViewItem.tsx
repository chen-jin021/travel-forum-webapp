...
interface ISquareTreeViewProps {
...
export const SquareTreeViewItem = ({
  node,
  type,
  title,
  childNodes,
  ...
  if (childNodes.length) {
    childrenItems = childNodes.map((child: RecursiveNodeTree) => {
      return changeUrlOnClick ? (
        <Link to={`/square/${pathToString(child.node.filePath)}`} key={child.node.nodeId}>
          <SquareTreeViewItem
            node={child.node}
  ...
  return (
    <div className="treeView-item">
      {changeUrlOnClick ? (
        <Link to={`/square/${pathToString(node.filePath)}`}>
          <TreeViewChild />
        </Link>
      ) : (
        <TreeViewChild />
