...
describe('Unit Test: findLinksByAnchorId', () => {
  ...
  test('gets links when given valid anchorId', async () => {
    ...
    const findLinkByAnchorIdResp = await linkCollectionConnection.findLinksByAnchorId(
      'anchor2'
    )
  ...
  test('success with empty payload array when given invalid anchorId', async () => {
    ...
    const findLinkByIdResp = await linkCollectionConnection.findLinksByAnchorId('anchor3')
